...
namespace Pagoda::Mirage {
    D3D12PipelineState::D3D12PipelineState(std::shared_ptr<D3D12Context> context, Shader* vertexShader, Shader* fragmentShader, VertexBufferLayout& vertexBufferLayout) : PipelineState(vertexShader, fragmentShader, vertexBufferLayout), m_ctx(context) {
        ...
        psoDesc.pRootSignature = this->m_ctx->GetRootSignature().Get();
        ...
        if (this->m_ctx->GetDevice()->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&m_pipelineState)) != S_OK) {
    ...
    void D3D12PipelineState::Bind() const {
        this->m_ctx->GetCommandList()->SetPipelineState(this->m_pipelineState.Get());
    }
